...
import com.tw.go.plugin.maven.config.ServerURL;
...
public class ArtifactoryPoller implements PackageMaterialPoller {
    private static Logger LOGGER = Logger.getLoggerFor(ArtifactoryPoller.class);
    ...
    public PackageRevision latestModificationSince(PackageConfiguration packageConfig, RepositoryConfiguration repoConfig, PackageRevision previouslyKnownRevision) {
        ...
        if(previouslyKnownRevision == null || previouslyKnownRevision.getTimestamp() == null){
            LOGGER.error("previouslyKnownRevision is null or has null timestamp");
    ...
    public Result checkConnectionToRepository(RepositoryConfiguration repoConfig) {
        ...
        try {
            ServerURL serverURL = new MavenRepoConfig(repoConfig).getServerUrl();
            if (!new RepositoryConnector().testConnection(serverURL.getCheckConnectionURL(), serverURL.getUser(), serverURL.getPassword())) {
    ...
    PackageRevision poll(LookupParams params) {
        MavenVersion latest = new RepositoryClient(params).getLatest();
